...
const DATADIR = app.reserve('data/journals_2018');
const STATE = {
  ...app.conf(), // replace, keyfile, project, dataset
  table   : 'raw_journals_2018',
  url     : 'https://web.archive.org.au/awa/20220309172834mp_/https://www.arc.gov.au/file/10549/download?token=Sbfb2a9n',
  dfile   : app.resolve(DATADIR,'era_2018_journal_list.xlsx'),
  ...
  schema  : { 
    ...
    fields : [
      ...
      { name:'weighted'  , type:'RECORD' , mode:'REPEATED', description:'Field of Research (FoR) codes assigned by ARC to the journal.', 
        fields : [ 
          { name:'code'  , type:'STRING' , mode:'REQUIRED', description:'FoR code (a string because the values are zero padded and can be "MD")' },
          { name:'weight', type:'INTEGER', mode:'REQUIRED', description:'Portional assignment of this FoR code (should sum to 100 for the 2-digit codes and 100 for the 4-digit codes)' },
        ]
      },
    ]
  }
